...
def _format_cell(row: dict) -> str:
    ...
    return (
        ...
        f"[{row['ci_lower']:+.3f}, {row['ci_upper']:+.3f}]{star}"
    )
...
def build_stats_table(
    ...
    *,
    ...
    if not runs["noise"]:
        sys.stderr.write(
            f"[build_stats_table] no noise records under "
            f"{runs_dir / 'noise'} — abort\n"
        )
        return 1
    ...
    rows = _stats_per_cell(cells, n_resamples=n_resamples, ci_level=ci_level)
    _apply_bh(rows, q=q)
