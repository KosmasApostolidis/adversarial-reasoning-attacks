...
def _compute_mean_ed_data(by_attack) -> tuple[list[float], np.ndarray, np.ndarray, list[int]]:
    ...
    ns = [edits(by_attack[a]).size for a in ATTACK_ORDER]
...
def _add_mean_ed_panel(fig, by_attack) -> None:
    ...
    ax.set_facecolor(BG)
    means, err_lo, err_hi, ns = _compute_mean_ed_data(by_attack)
    _draw_mean_ed_bars(ax, means, err_lo, err_hi, ns)
    _decorate_mean_ed_axes(ax, means)


def _draw_one_eps_curve(ax, name: str, recs: list[dict]) -> None:
...
def _add_eps_curves_panel(fig, by_attack) -> None:
    ...
    for name in ATTACK_ORDER:
        _draw_one_eps_curve(ax, name, by_attack[name])
    for e in eps_vals:
...
def _add_flip_bars_panel(fig, by_attack) -> None:
...
def fig_bento(by_attack, out_path: Path) -> None:
    ...
    _add_header(fig)
    _add_strongest_kpi(fig, by_attack)
    _add_flipper_kpi(fig, by_attack)
    _add_total_records_kpi(fig, by_attack)
    _add_gpu_time_kpi(fig)
    _add_mean_ed_panel(fig, by_attack)
    _add_eps_curves_panel(fig, by_attack)
    _add_flip_bars_panel(fig, by_attack)
    _add_footer(fig)
