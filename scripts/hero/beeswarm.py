...
def _add_card_header(fig, card_x: float) -> None:
    ...
    fig.text(card_x, 0.918, "n samples · mean ed [95% CI] · step-1 flip rate",
             color=TEXT_MUTED, fontsize=10)


def _draw_card_background(fig, geom: _CardGeom, color: str) -> None:
    add_panel(fig, geom.x, geom.y_top - geom.height, geom.width, geom.height,
              fc=PANEL, ec=GRID, radius=0.012)
    stripe = Rectangle((geom.x, geom.y_top - geom.height), 0.005, geom.height,
                       facecolor=color, edgecolor="none", transform=fig.transFigure)
    fig.patches.append(stripe)


def _draw_card_stats_text(fig, geom: _CardGeom, name: str, eds: np.ndarray, lo: float, hi: float) -> None:
    fig.text(geom.x + 0.018, geom.y_top - 0.022, LABELS[name],
             color=PALETTE[name], fontsize=11, fontweight="bold")
    fig.text(geom.x + 0.018, geom.y_top - 0.062, f"μ = {eds.mean():.3f}",
             color=TEXT, fontsize=18, fontweight="bold", family="DejaVu Sans Mono")
    fig.text(geom.x + 0.018, geom.y_top - 0.083,
...
def _draw_flip_bar(fig, geom: _CardGeom, flip: float, color: str) -> None:
    ...
    fig.patches.append(Rectangle((bar_x, bar_y), bar_w, 0.008,
                                  facecolor=PANEL_LIGHT, edgecolor="none",
                                  transform=fig.transFigure))
    fig.patches.append(Rectangle((bar_x, bar_y), bar_w * flip, 0.008,
                                  facecolor=color, edgecolor="none",
                                  transform=fig.transFigure))
    fig.text(geom.x + 0.018, bar_y + 0.012, f"step-1 flip {flip:.0%}",
...
def _draw_stat_card(fig, geom: _CardGeom, name: str, recs: list[dict]) -> None:
    ...
    eds = edits(recs)
    if eds.size == 0:
        fig.text(geom.x + 0.018, geom.y_top - geom.height / 2, "no data",
                 color=TEXT_MUTED, fontsize=10)
        return
...
def _draw_legend(fig) -> None:
    ...
    fig.add_artist(plt.Line2D([0.105, 0.125], [leg_y + 0.005, leg_y + 0.005],
                              color=TEXT, linewidth=1.5, transform=fig.transFigure))
    fig.text(0.135, leg_y, "MEAN", color=TEXT_MUTED, fontsize=8.5)
    fig.add_artist(plt.Line2D([0.165], [leg_y + 0.005], marker="D", markersize=8,
                              color=ACCENT, markeredgecolor=BG, markeredgewidth=1.0,
                              transform=fig.transFigure))
...
def fig_beeswarm(by_attack, out_path: Path) -> None:
    ...
    for i, name in enumerate(ATTACK_ORDER):
        geom = _CardGeom(x=card_x, width=card_w, height=card_h,
                         y_top=card_y0 - i * (card_h + card_gap))
