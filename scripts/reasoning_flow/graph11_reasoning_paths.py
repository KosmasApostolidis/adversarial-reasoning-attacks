...
def _draw_path_arrows(ax, seq: list[str], tool_pos: dict, color: str) -> None:
    n = len(seq)
...
def _decorate_path_axes(
    ax, row: int, col: int, ed: float, title: str, color: str, pid: str
) -> None:
    ax.set_xlim(-1.5, 1.5)
    ax.set_ylim(-1.5, 1.5)
    ax.set_aspect("equal")
...
def _render_subplot(
    ...
    _draw_background_nodes(ax, all_tools, tool_pos)

    # Highlight visited tools
    _draw_visited_nodes(ax, seq, b_seq, tool_pos, color)

    # Draw path as gradient-colored arrows
    _draw_path_arrows(ax, seq, tool_pos, color)

    _decorate_path_axes(ax, row, col, rec["edit_distance_norm"], title, color, pid)
...
def graph11_reasoning_paths():
    ...
    tool_pos = _circular_tool_layout(all_tools)
    ...
    for col, rec in enumerate(pgd_r):
        ...
        for row, (key, color, title) in enumerate(
            ...
            ax = axes[row, col]
            _render_subplot(ax, rec, key, color, title, row, col, pid, all_tools, tool_pos)
