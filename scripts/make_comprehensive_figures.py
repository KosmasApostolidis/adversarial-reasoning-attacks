...
def stat1_overview():
    ...
    bp = ax.boxplot(data, positions=[1, 2, 3], widths=0.18, patch_artist=True,
    ...
    for xi, arr, c in zip([1, 2, 3], data, colors):
    ...
    ax.scatter(b_lens_n, nd,  s=90, color=C_NOISE, edgecolors="white", lw=0.8,
               label="Noise", zorder=4, alpha=0.9)
    ax.scatter(b_lens_p, pd_, s=90, color=C_PGD,   edgecolors="white", lw=0.8,
               label="PGD",   zorder=4, alpha=0.9)
    # regression lines
    for xs, ys, c in [(b_lens_n, nd, C_NOISE), (b_lens_p, pd_, C_PGD)]:
        if len(set(xs)) > 1:
            m, b, r, *_ = scipy_stats.linregress(xs, ys)
            xr = np.linspace(min(xs), max(xs), 50)
            ax.plot(xr, m * xr + b, color=c, lw=1.8, alpha=0.6, linestyle="--",
                    label=f"r={r:.2f}")
...
def stat2_epsilon_sweep():
    ...
    ax = axes[0]
    ax.errorbar([e*255 for e in eps_vals], means, yerr=sems,
                fmt="-o", color=C_NOISE, lw=2.5, ms=9, capsize=5,
                label="mean ± SEM (noise, Qwen)")
    ax.fill_between([e*255 for e in eps_vals],
                    [m - s for m, s in zip(means, sems)],
                    [m + s for m, s in zip(means, sems)],
                    alpha=0.18, color=C_NOISE)
    ...
    ax.axhline(pgd_r[0]["edit_distance_norm"] if False else
               np.mean([r["edit_distance_norm"] for r in pgd_r]),
               color=C_PGD, lw=2, linestyle="--",
    ...
    xs = [1, 2]
    for xi, (label, vals, c) in zip(xs, groups):
        ax.bar(xi, np.mean(vals), width=0.5, color=c, alpha=0.75, edgecolor="white", lw=1.5,
               yerr=scipy_stats.sem(vals), capsize=7,
    ...
    ax.text(1.5, y_max + 0.03, "★  3× more drift", ha="center", fontsize=10.5,
            fontweight="bold", color=C_PGD)
...
def stat4_step_heatmap():
    ...
    for ax, (title, mat, c) in zip(axes, conditions):
        cmap = LinearSegmentedColormap.from_list(f"cm_{c}",
               ["#f7f7f7", c], N=256)
        ...
        ax.set_xlabel("Step position"); ax.set_title(title, pad=8, fontsize=11)
        for i in range(len(all_tools)):
            for j in range(MAX_STEP):
                v = mat[i, j]
                if v > 0:
                    ax.text(j, i, str(int(v)), ha="center", va="center", fontsize=9,
                            color="white" if v > mat.max()*0.55 else "black", fontweight="bold")
        fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04).set_label("Count", fontsize=8)

    fig.suptitle("Tool occupancy at each trajectory step: how PGD rewires step-by-step reasoning",
                 fontsize=13, fontweight="bold", y=1.02)
...
def graph6_bipartite():
    ...
    for ax, rec in zip(axes, pgd_r):
        ...
        # Draw nodes
        for col, seq, xpos in [(b, "left", 0.15), (a, "right", 0.85)]:
            for step, tool in enumerate(seq):
                y = 1 - (step + 0.5) / max_len
                fc = C_BENIGN if xpos < 0.5 else C_PGD
                circle = plt.Circle((xpos, y), 0.045, color=fc, zorder=4,
                                    linewidth=2)
                circle.set_edgecolor("white")
                ax.add_patch(circle)
                ax.text(xpos, y, _s(tool), ha="center", va="center",
                        fontsize=6.5, color="white", fontweight="bold", zorder=5,
                        path_effects=[pe.withStroke(linewidth=1.5, foreground=DARK_BG)])
            # column header
            ax.text(xpos, 1.04, "Benign" if xpos < 0.5 else "PGD",
                    ha="center", fontsize=9, color=C_BENIGN if xpos < 0.5 else C_PGD,
                    fontweight="bold")
        ...
        for i in range(max_len):
            ...
            if bt and at:
                ...
                ax.annotate("", xy=(0.83, ay), xytext=(0.23, by),
                            arrowprops=dict(arrowstyle="-|>", color=color,
                                            lw=lw, mutation_scale=10),
                            alpha=alpha)
            elif bt and not at:
                ax.text(0.5, by, "✕", ha="center", va="center",
        ...
        ax.set_title(f"P{pid}\ned={rec['edit_distance_norm']:.3f}",
                     fontsize=9, color=DARK_FG, pad=4)
    ...
    fig.legend(handles=handles, loc="lower center", ncol=3,
               ...
               labelcolor=DARK_FG)
    fig.suptitle("Bipartite trajectory alignment: benign ↔ PGD-attacked (per patient, per step)",
                 fontsize=13, fontweight="bold", color=DARK_FG, y=1.01)
    fig.savefig(GRAPH_OUT / "graph6_bipartite.png", bbox_inches="tight",
                facecolor=DARK_BG)
    plt.close(fig)
    print("graph6 ✓")
...
def graph7_divergence():
    ...
    for ax, rec in zip(axes, pgd_r):
        ...
        for step in range(max_len):
            ...
            if step == div_step and div_step < min(len(b), len(a)):
                ax.axhline(y + 0.5 / max_len, color=C_PGD, lw=1.5,
                           linestyle="--", alpha=0.6, xmin=0.1, xmax=0.9)
                ax.text(0.95, y + 0.5 / max_len, "⚡", fontsize=12, va="center",
                        color=C_PGD, zorder=6)
                ...
                for seq, xpos, c in [(b, 0.3, C_BENIGN), (a, 0.7, C_PGD)]:
                    if step < len(seq) and step - 1 < len(seq):
                        ...
                        if not (prev_merged and curr_merged and c == C_PGD):
                            ax.plot([px, cx], [py, y], color=c, lw=1.5,
                                    alpha=0.55, zorder=2)
    ...
    fig.suptitle("Divergence tree: where adversarial attack splits the reasoning chain",
                 fontsize=13, fontweight="bold", color=DARK_FG, y=1.01)
    fig.savefig(GRAPH_OUT / "graph7_divergence.png", bbox_inches="tight",
                facecolor=DARK_BG)
    plt.close(fig)
    print("graph7 ✓")
...
def graph8_tool_influence():
    ...
    for t in all_tools:
        ...
        if s > 0:
            for radius_scale, alpha in [(0.16, 0.06), (0.12, 0.12), (0.08, 0.22)]:
                glow = plt.Circle(pos[t], radius_scale + 0.02,
                                  color=C_PGD, alpha=alpha * s, zorder=1)
                ax.add_patch(glow)
    ...
    for t in all_tools:
        ...
        if s > 0:
            ax.text(pos[t][0], pos[t][1] - sz - 0.06,
                    f"sens={s:.0%}", ha="center", fontsize=7.5,
                    color=cmap_sens(s), zorder=6)

    sm = plt.cm.ScalarMappable(cmap=cmap_sens, norm=mpl.colors.Normalize(0, 1))
    sm.set_array([])
    cb = fig.colorbar(sm, ax=ax, fraction=0.03, pad=0.02)
    cb.set_label("Adversarial sensitivity (frac. positions changed)", fontsize=9,
                 color=DARK_FG)
    ...
    ax.set_title("Tool influence graph — glow + colour = adversarial sensitivity under PGD",
                 fontsize=13, fontweight="bold", color=DARK_FG, pad=12)
    fig.savefig(GRAPH_OUT / "graph8_tool_influence.png", bbox_inches="tight",
                facecolor=DARK_BG)
    plt.close(fig)
    print("graph8 ✓")
...
def graph9_layered_flow():
    ...
    ax.legend(handles=handles, loc="upper right", fontsize=10,
              framealpha=0.3, facecolor=DARK_AX, edgecolor=DARK_GRID,
              labelcolor=DARK_FG)
    ax.set_title(
        "Layered flow: how PGD shifts tool selection at each reasoning step (n=5 patients)",
        fontsize=13, fontweight="bold", color=DARK_FG, pad=12,
    )
    fig.savefig(GRAPH_OUT / "graph9_layered_flow.png", bbox_inches="tight",
                facecolor=DARK_BG)
    plt.close(fig)
    print("graph9 ✓")
...
def graph10_step_occupancy():
    ...
    for ax, title, mat, cmap, vmin, vmax in zip(axes, titles, mats, cmaps, vmins, vmaxs):
        ...
        ax.set_xticks(range(MAX_STEP))
        ax.set_xticklabels([f"s{i+1}" for i in range(MAX_STEP)],
    ...
    fig.suptitle("Step-position occupancy heatmap — which tools PGD inserts, removes, or shifts",
                 fontsize=13, fontweight="bold", color=DARK_FG, y=1.02)
    fig.savefig(GRAPH_OUT / "graph10_step_occupancy.png", bbox_inches="tight",
                facecolor=DARK_BG)
    plt.close(fig)
    print("graph10 ✓")
