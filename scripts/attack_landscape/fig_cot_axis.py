...
def _plot_one_cot_attack(ax, name: str, recs: list[dict]) -> bool:
...
def _decorate_cot_axis(ax, eps_vals: list[float]) -> None:
...
def fig_cot_axis(by_attack: dict[str, list[dict]], out_path: Path) -> None:
    ...
    for name in ATTACK_ORDER:
        if _plot_one_cot_attack(ax, name, by_attack[name]):
            plotted_any = True
    ...
    _decorate_cot_axis(ax, eps_vals)
    plt.tight_layout()
