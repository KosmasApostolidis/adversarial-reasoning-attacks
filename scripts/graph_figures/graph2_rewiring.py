...
def _build_freq_counters(records: list[dict], key: str) -> tuple[Counter, Counter]:
...
def _circular_layout(tools: list[str]) -> dict[str, tuple[float, float]]:
    angles = np.linspace(0, 2 * np.pi, len(tools), endpoint=False)
    return {t: (np.cos(a) * 1.4, np.sin(a) * 1.4) for t, a in zip(tools, angles, strict=False)}


def _draw_panel_edges(ax, G: nx.DiGraph, pos: dict, ec: str) -> None:
...
def _draw_panel_nodes(ax, tools: list[str], pos: dict, nf: Counter, ec: str) -> None:
    for t in tools:
...
def _render_rewiring_panel(
    ax, tools: list[str], pos: dict, nf: Counter, ef: Counter, ec: str, title: str
) -> None:
    G = nx.DiGraph()
    ...
    # Edges
    _draw_panel_edges(ax, G, pos, ec)

    # Nodes
    _draw_panel_nodes(ax, tools, pos, nf, ec)
...
def graph2_rewiring() -> None:
    ...
    bn_freq, be_freq = _build_freq_counters(pgd_recs, "benign")
    pn_freq, pe_freq = _build_freq_counters(pgd_recs, "attacked")

    # Fixed circular layout for both panels
    pos = _circular_layout(tools)
    ...
    for ax, title, nf, ef, ec in zip(axes, titles, nf_list, ef_list, colors, strict=False):
        _render_rewiring_panel(ax, tools, pos, nf, ef, ec, title)
