...
def add_panel(fig_or_ax, x, y, w, h, *, fc=PANEL, ec=GRID, alpha=1.0, radius=0.012):
    ...
    box = FancyBboxPatch(
        ...
        linewidth=0.8, edgecolor=ec, facecolor=fc, alpha=alpha,
...
def fig_beeswarm(by_attack, out_path: Path) -> None:
    ...
    for i, name in enumerate(ATTACK_ORDER):
        ...
        ax.scatter(vals, np.full_like(vals, i, dtype=float) + jitter,
                   s=85, color=color, alpha=0.78, edgecolor=BG, linewidth=0.9, zorder=3)
        # Median line
    ...
    ax.set_yticklabels([LABELS[a] for a in ATTACK_ORDER], color=TEXT, fontsize=12, fontweight="bold")
    ax.invert_yaxis()
    ...
    fig.text(0.06, 0.918, "Trajectory edit distance per record · Qwen2.5-VL-7B medical agent · ProstateX val=5",
    ...
    fig.text(card_x, 0.918, "n samples · mean ed [95% CI] · step-1 flip rate", color=TEXT_MUTED, fontsize=10)

    for i, name in enumerate(ATTACK_ORDER):
        y = card_y0 - i * (card_h + card_gap)
        add_panel(fig, card_x, y - card_h, card_w, card_h, fc=PANEL, ec=GRID, radius=0.012)
        # Color stripe left
        stripe = Rectangle((card_x, y - card_h), 0.005, card_h, facecolor=PALETTE[name], edgecolor="none", transform=fig.transFigure)
        ...
        # Heading
        fig.text(card_x + 0.018, y - 0.022, LABELS[name], color=PALETTE[name], fontsize=11, fontweight="bold")
        # Big mean number
        fig.text(card_x + 0.018, y - 0.062, f"μ = {eds.mean():.3f}",
                 color=TEXT, fontsize=18, fontweight="bold", family="DejaVu Sans Mono")
        # Detail
        fig.text(card_x + 0.018, y - 0.083,
        ...
        # Track
        fig.patches.append(Rectangle((bar_x, bar_y), bar_w, 0.008, facecolor=PANEL_LIGHT, edgecolor="none", transform=fig.transFigure))
        # Fill
        fig.patches.append(Rectangle((bar_x, bar_y), bar_w * flip, 0.008, facecolor=PALETTE[name], edgecolor="none", transform=fig.transFigure))
        fig.text(card_x + 0.018, bar_y + 0.012, f"step-1 flip {flip:.0%}", color=TEXT_MUTED, fontsize=8.5)

    # Legend strip below main plot — placed below xlabel
    leg_y = 0.065
    fig.text(0.06, leg_y, "MEDIAN", color=TEXT_MUTED, fontsize=8.5)
    fig.add_artist(plt.Line2D([0.105, 0.125], [leg_y + 0.005, leg_y + 0.005], color=TEXT, linewidth=1.5, transform=fig.transFigure))
    fig.text(0.135, leg_y, "MEAN", color=TEXT_MUTED, fontsize=8.5)
    fig.add_artist(plt.Line2D([0.165], [leg_y + 0.005], marker="D", markersize=8, color=ACCENT, markeredgecolor=BG, markeredgewidth=1.0, transform=fig.transFigure))
    fig.text(0.185, leg_y, "EACH DOT = ONE RECORD (sample × ε × seed)",
             color=TEXT_MUTED, fontsize=8.5)

    # Footer
    fig.text(0.06, 0.018, "adversarial-reasoning-attacks · 2026 · github.com/KosmasApostolidis/adversarial-reasoning-attacks",
             color=TEXT_MUTED, fontsize=8, alpha=0.6)
    fig.text(0.96, 0.018, "ε ∈ {2, 4, 8, 16}/255 · seeds {0, 1, 2}",
             color=TEXT_MUTED, fontsize=8, alpha=0.6, ha="right")
...
def fig_ridgeline(by_attack, out_path: Path) -> None:
    ...
    for i, name in enumerate(ATTACK_ORDER):
        ...
        ax.fill_between(xs, y_base, y_base + density * 0.85, color=rgba_fill, edgecolor=rgba_edge, linewidth=2.0, zorder=3 - i * 0.05)
        # Median + mean tick
        med = float(np.median(vals))
        mn  = float(np.mean(vals))
        ax.plot([med, med], [y_base, y_base + 0.22], color=TEXT, linewidth=1.8, zorder=8)
        ax.scatter([mn], [y_base], marker="D", s=70, color=ACCENT, edgecolor=BG, linewidth=1.0, zorder=9)
        # Label inside ridge
        ax.text(-0.05, y_base + 0.06, LABELS[name], color=color, fontsize=12, fontweight="bold", ha="left", va="bottom")
        ax.text(1.05, y_base + 0.06, f"n={vals.size}  μ={mn:.3f}", color=TEXT_MUTED, fontsize=10, ha="right", va="bottom", family="DejaVu Sans Mono")
    ...
    fig.text(0.10, 0.918, "Kernel density estimate of edit-distance per attack · vertical tick = median · diamond = mean",
             color=TEXT_MUTED, fontsize=11)
    fig.text(0.10, 0.04, "More mass near 1.0 = trajectory rewritten · more mass near 0 = agent unaffected",
             color=TEXT_MUTED, fontsize=9.5, alpha=0.85)
...
def fig_heatmap(by_attack, out_path: Path) -> None:
    ...
    for i in range(masked.shape[0]):
        for j in range(masked.shape[1]):
            if np.isnan(cell[i, j]):
                ax.text(j, i, "n/a", ha="center", va="center", color=TEXT_MUTED, fontsize=10, fontstyle="italic")
            else:
                v = cell[i, j]
                color = "black" if v > 0.55 else TEXT
                ax.text(j, i - 0.10, f"{v:.3f}", ha="center", va="center",
                        color=color, fontsize=14, fontweight="bold", family="DejaVu Sans Mono")
    ...
    ax.set_yticklabels([LABELS[a] for a in attacks_with_data], color=TEXT, fontsize=11, fontweight="bold")
    ax.set_xlabel("Perturbation budget ε", color=TEXT_MUTED, fontsize=12)
    ax.tick_params(length=0)

    cbar_ax = fig.add_axes([0.86, 0.20, 0.018, 0.55])
    cbar = fig.colorbar(im, cax=cbar_ax)
    cbar.outline.set_edgecolor(GRID); cbar.outline.set_linewidth(0.7)
    ...
    fig.text(0.06, 0.895, "Mean normalised edit distance per (attack, ε) cell · brighter = more disruption",
             color=TEXT_MUTED, fontsize=11)
    fig.text(0.06, 0.05, "PGD evaluated only at smoke ε=8/255 (n=5); other attacks span full sweep (4 ε × 3 seeds × 5 samples = 60).",
...
def fig_radial(by_attack, out_path: Path) -> None:
    ...
    def metrics(name):
        ...
        return [float(eds.mean()), float(eds.max()), step1_flip_rate(recs),
                float(np.quantile(eds, 0.95)), float(traj_d)]
    ...
    for idx, name in enumerate(ATTACK_ORDER):
        ...
        bars = ax.bar(angles, norm[idx], width=bar_w, color=PALETTE[name],
                      alpha=0.92, edgecolor=BG, linewidth=1.6, zorder=3)
        # Annotate raw values just outside the bar tip
        for i_m, (ang, val_norm, val_raw) in enumerate(zip(angles, norm[idx], raw[name], strict=True)):
            label = f"{val_raw:.0%}" if metric_names[i_m] == "FLIP" else f"{val_raw:.2f}"
            ax.text(ang, max(val_norm + 0.10, 0.18), label, ha="center", va="center",
                    color=TEXT, fontsize=10, fontweight="bold", family="DejaVu Sans Mono", zorder=5)
    ...
    ax.text(0.06, 0.71, "Bar length = column-normalised score across\nall attacks (longer = stronger attack).",
    ...
    ax.text(0.06, 0.06, "Numbers shown above each bar are raw values\n(not normalised).",
    ...
    fig.text(0.5, 0.940, "Five-axis radar per attack · column-normalised across attacks",
...
def fig_bento(by_attack, out_path: Path) -> None:
    ...
    fig.text(0.04, 0.928, "Qwen2.5-VL-7B · ProstateX val=5 · ε ∈ {2,4,8,16}/255 · seeds {0,1,2}",
             color=TEXT_MUTED, fontsize=11)

    # ── Cell A: Hero stat — strongest attack
    add_panel(fig, 0.04, 0.66, 0.24, 0.24, fc=PANEL, ec=GRID, radius=0.018)
    strongest = max(ATTACK_ORDER, key=lambda a: edits(by_attack[a]).mean() if edits(by_attack[a]).size else 0)
    ...
    fig.text(0.06, 0.720, "mean normalised edit distance",
    ...
    fig.text(0.32, 0.795, f"{flip_v:.0%}", color=TEXT, fontsize=44, fontweight="bold",
             family="DejaVu Sans Mono", va="top")
    fig.text(0.32, 0.720, "of trajectories had the first tool flipped",
             color=TEXT_MUTED, fontsize=9.5)
    ...
    fig.text(0.58, 0.815, f"{total}", color=ACCENT, fontsize=54, fontweight="bold",
             family="DejaVu Sans Mono", va="top")
    fig.text(0.58, 0.720, f"across {sum(1 for a in ATTACK_ORDER if by_attack[a])} attack modes",
             color=TEXT_MUTED, fontsize=9.5)
    ...
    fig.text(0.80, 0.815, "46:00", color=PALETTE["targeted_tool"], fontsize=54, fontweight="bold",
             family="DejaVu Sans Mono", va="top")
    fig.text(0.80, 0.720, "minutes : seconds (sequential)",
             color=TEXT_MUTED, fontsize=9.5)
    ...
    bars = ax.barh(ypos, means, color=[PALETTE[a] for a in ATTACK_ORDER],
                   alpha=0.92, edgecolor=BG, linewidth=1.0, height=0.62)
    ax.errorbar(means, ypos, xerr=[err_lo, err_hi], fmt="none", color=TEXT, capsize=5, linewidth=1.3, alpha=0.85)
    for i, (m, n) in enumerate(zip(means, [edits(by_attack[a]).size for a in ATTACK_ORDER], strict=True)):
        ax.text(m + 0.02, i, f"{m:.3f}  ", color=TEXT, fontsize=11, va="center", fontweight="bold", family="DejaVu Sans Mono")
        ax.text(m + 0.02, i + 0.35, f"n={n}", color=TEXT_MUTED, fontsize=8.5, va="center", family="DejaVu Sans Mono")
    ax.set_yticks(ypos); ax.set_yticklabels([LABELS[a] for a in ATTACK_ORDER], color=TEXT, fontsize=10.5, fontweight="bold")
    ...
    ax.invert_yaxis()
    ax.set_title("MEAN EDIT DISTANCE PER ATTACK", color=TEXT, fontsize=12, fontweight="bold", loc="left", pad=12)
    ...
    for name in ATTACK_ORDER:
        ...
        if len(xs) == 1:
            ax.scatter(xs, ys_mean, marker="*", s=240, color=PALETTE[name], edgecolor=BG, linewidth=1.2, label=f"{LABELS[name]}*", zorder=4)
        else:
            ax.plot(xs, ys_mean, marker="o", color=PALETTE[name], linewidth=2.5,
                    markersize=8, markeredgecolor=BG, markeredgewidth=0.8, label=LABELS[name], zorder=3)
    ...
    ax.set_xticks(eps_vals); ax.set_xticklabels([fmt_eps(e) for e in eps_vals], color=TEXT_MUTED)
    ax.set_xlabel("Perturbation budget ε", color=TEXT_MUTED, fontsize=10)
    ax.set_ylabel("Mean ed", color=TEXT_MUTED, fontsize=10)
    ax.set_title("EFFECTIVENESS VS ε", color=TEXT, fontsize=12, fontweight="bold", loc="left", pad=12)
    ...
    ypos = np.arange(len(ATTACK_ORDER))
    ax.barh(ypos, flips, color=[PALETTE[a] for a in ATTACK_ORDER], alpha=0.92, edgecolor=BG, linewidth=1.0, height=0.62)
    for i, f in enumerate(flips):
        ax.text(f + 0.012, i, f"{f:.0%}", color=TEXT, fontsize=10, va="center", fontweight="bold", family="DejaVu Sans Mono")
    ax.set_yticks(ypos); ax.set_yticklabels([LABELS[a] for a in ATTACK_ORDER], color=TEXT, fontsize=9.5)
    ax.set_xlim(0, 1.05); ax.set_xticks(np.linspace(0, 1, 6))
    for v in np.linspace(0, 1, 6):
        ax.axvline(v, color=GRID, linewidth=0.4, alpha=0.5, zorder=0)
    ax.set_xlabel("Step-1 flip rate", color=TEXT_MUTED, fontsize=10)
    ax.invert_yaxis()
    ax.set_title("STEP-1 TOOL-FLIP RATE", color=TEXT, fontsize=12, fontweight="bold", loc="left", pad=10)

    # Footer
    fig.text(0.04, 0.025, "github.com/KosmasApostolidis/adversarial-reasoning-attacks   · 2026",
             color=TEXT_MUTED, fontsize=8.5, alpha=0.7)
    fig.text(0.96, 0.025, "* PGD anchor: smoke ε=8/255 only (n=5)", color=TEXT_MUTED, fontsize=8.5, alpha=0.7, ha="right")
...
def main() -> int:
    ...
    for name in ATTACK_ORDER:
        ...
        print(f"  {name:18s} n={eds.size:3d}  μ={eds.mean():.3f}  flip={step1_flip_rate(by_attack[name]):.0%}")
    return 0
